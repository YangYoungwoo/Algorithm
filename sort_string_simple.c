...
void insertion_sort_string ( struct String arr[], int n ) {
  /* FILL */
  //source: https://cboard.cprogramming.com/c-programming/101186-insertion-sort-array-structs.html
  //source: https://blog.naver.com/serverwizard/220778549735
  int i;
  char temp[MAX_STR_LEN];
  for(i = 1; i < n; i++){
    strcpy(temp,  arr[i].s);
    while(i > 0 && strcmp(arr[i-1].s,temp)>0){
        strcpy(arr[i].s, arr[i-1].s);
        i = i - 1;
    }
    strcpy(arr[i].s, temp);
  }
...
void selection_sort_string(struct String arr[], const int n) {
  /* FILL */
  //source: https://cafe.naver.com/funcc/31562
  int i, j;
  int temp[MAX_STR_LEN];

  for(i = 0; i < n-1; i++){
    for(j = i+1; j < n; j++){
        if (strcmp(arr[j].s, arr[i].s) < 0){
            strcpy(temp, arr[i].s);
...
void bubble_sort_string(struct String arr[], int n) {
  /* FILL */
  //source: from sort_chararray_simple.c
  int i, j;
  char temp[MAX_STR_LEN];
  for (i=n-1; i>0; i--) {
    for (j=0; j<i; j++) {
